...
export const AdvantagesSection = () => {
  ...
  const trackDownloadClick = () => {
    window.dispatchEvent(
      new CustomEvent("download_pdf", {
        detail: { document: "comparison_report" },
      }),
    );
  };
  ...
  const advantagesTitle =
    t("advantages.title") ||
    "Compare Legal Nexus AI with Traditional Solutions";
  const advantagesSubtitle =
    t("advantages.subtitle") ||
    "See how our AI-powered solution compares to traditional legal research and document analysis methods";
  const ctaText = t("advantages.cta") || "Download Full Comparison Report";

  // Ensure the headers and rows are arrays
  const headers = Array.isArray(t("advantages.table.headers"))
    ? t("advantages.table.headers")
    : [];
  const rows = Array.isArray(t("advantages.table.rows"))
    ? t("advantages.table.rows")
    : [];

  return (
    <section
      className="py-20 md:py-28 bg-gradient-to-b from-neutral-50 to-white relative"
      ref={sectionRef}
    >
              ...
              <TableBody>
                {rows.map((row: string[], index: number) => (
                  <TableRow
                    key={index}
                    className={
                      index % 2 === 0 ? "bg-white" : "bg-neutral-softGray/30"
                    }
                  >
                    {Array.isArray(row)
